...
# deepreefmap-gui-macos-arm64. The PyApp flow below is otherwise platform-agnostic.
OUTPUT_NAME="${1:-${OUTPUT_NAME:-deepreefmap-gui-linux-x64}}"
TORCH_VARIANT="${2:-${TORCH_VARIANT:-default}}"

# Constants shared with build.ps1 (PyApp version, torch indexes).
...
WHEEL=$(ls dist/deepreefmap_gui-*-py3-none-any.whl)
VERSION=${WHEEL#dist/deepreefmap_gui-}; VERSION=${VERSION%-py3-none-any.whl}
...
TORCH_INDEX="${!idx_var:-}"
FEATURES="${TORCH_INDEX:+${TORCH_VARIANT}}"
...
# (see build.ps1).
PYAPP_PROJECT_NAME=deepreefmap-gui \
