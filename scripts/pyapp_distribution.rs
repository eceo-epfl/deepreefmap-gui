// Vendored from PyApp v0.29.0 `src/distribution.rs`. Copied over the cloned
// PyApp checkout during the build (scripts/build.sh, scripts/build.ps1), the
// same mechanism as scripts/pyapp_process.rs.
//
// The ONLY change from upstream is `ensure_ready` (see the "DEEPREEFMAP PATCH"
// markers below): it writes a `.pyapp-install-complete` marker after a fully
// successful install and, on the next launch, wipes any install_dir that exists
// without that marker. This makes a first-run install that was killed mid-way
// (Ctrl-C, closed terminal, laptop lid, power loss) self-heal instead of
// poisoning the env forever with `ModuleNotFoundError: No module named
// 'deepreefmap_gui'`.
//
// Re-sync this file from upstream whenever PYAPP_VER in scripts/build_config.env
// changes (same maintenance contract as scripts/pyapp_process.rs).
use std::env;
use std::ffi::OsStr;
use std::fs;
use std::io::Write;
...
pub fn ensure_ready() -> Result<()> {
    ...
    // ---- BEGIN DEEPREEFMAP PATCH (self-healing partial installs) ----
    // A first-run install that was killed mid-way (Ctrl-C, closed terminal,
    // laptop lid, power loss) leaves install_dir present but with the project
    // uninstalled -- stock PyApp's `!install_dir.is_dir()` guard then treats it
    // as installed and every later launch fails with ModuleNotFoundError. The
    // marker below is written only after materialize + install_project both
    // succeed, so "install_dir exists without the marker" means a poisoned
    // partial install: wipe it here (under the install lock already held) so the
    // block below reinstalls from scratch.
    let marker = app::install_dir().join(".pyapp-install-complete");
    if app::install_dir().is_dir() && !marker.is_file() {
        fs::remove_dir_all(app::install_dir()).ok();
    }
    // ---- END DEEPREEFMAP PATCH ----
    ...
    if !app::install_dir().is_dir() {
        ...
        // ---- BEGIN DEEPREEFMAP PATCH (self-healing partial installs) ----
        // Reached only when materialize + install_project both succeeded (they
        // exit() on a non-zero setup command via check_setup_status), so the
        // marker's presence is proof of a complete environment.
        fs::File::create(&marker)
            .with_context(|| format!("unable to write install marker {}", marker.display()))?;
        // ---- END DEEPREEFMAP PATCH ----
    }
